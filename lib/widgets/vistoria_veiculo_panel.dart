...
class _VistoriaVeiculoPanelState extends State<VistoriaVeiculoPanel>{
  ...
  @override Widget build(BuildContext context){final item=widget.item;return Column(children:[
    ...
    sec('Declarações e observações',[CheckboxListTile(contentPadding:EdgeInsets.zero,value:v.proprietarioOrientado,title:const Text('O proprietário foi orientado a retirar os pertences do veículo.'),onChanged:(x)=>setState(()=>emit(v.copyWith(proprietarioOrientado:x??false)))),CheckboxListTile(contentPadding:EdgeInsets.zero,value:v.patioCiente,title:const Text('Ciente de que o veículo removido para o pátio poderá gerar cobrança de diárias.'),onChanged:(x)=>setState(()=>emit(v.copyWith(patioCiente:x??false)))),TextField(controller:obs,minLines:3,maxLines:6,onChanged:(x)=>emit(v.copyWith(observacoes:x)),decoration:dec('Observações'))]),
    sec('Segurado ou beneficiário',[TextFormField(initialValue:v.seguradoNome,onChanged:(x)=>emit(v.copyWith(seguradoNome:x)),decoration:dec('Nome')),const SizedBox(height:8),TextFormField(initialValue:v.seguradoRg,onChanged:(x)=>emit(v.copyWith(seguradoRg:x)),decoration:dec('RG')),const SizedBox(height:8),TextFormField(initialValue:v.seguradoAssinatura,onChanged:(x)=>emit(v.copyWith(seguradoAssinatura:x)),decoration:dec('Assinatura / confirmação'))]),
    sec('Destinatário',[TextFormField(initialValue:v.destinatarioNome,onChanged:(x)=>emit(v.copyWith(destinatarioNome:x)),decoration:dec('Nome')),const SizedBox(height:8),TextFormField(initialValue:v.destinatarioRg,onChanged:(x)=>emit(v.copyWith(destinatarioRg:x)),decoration:dec('RG')),const SizedBox(height:8),TextFormField(initialValue:v.destinatarioAssinatura,onChanged:(x)=>emit(v.copyWith(destinatarioAssinatura:x)),decoration:dec('Assinatura / confirmação'))]),
    sec('Prestador',[TextFormField(initialValue:v.prestadorNome,onChanged:(x)=>emit(v.copyWith(prestadorNome:x)),decoration:dec('Nome')),const SizedBox(height:8),TextFormField(initialValue:v.prestadorRg,onChanged:(x)=>emit(v.copyWith(prestadorRg:x)),decoration:dec('RG')),const SizedBox(height:8),TextFormField(initialValue:v.prestadorAssinatura,onChanged:(x)=>emit(v.copyWith(prestadorAssinatura:x)),decoration:dec('Assinatura / confirmação'))]),
  ]);}
}
